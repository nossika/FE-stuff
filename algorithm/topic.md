...
function avoidFlood(rains: number[]): number[] {
    ...
    // 二分法找出 arr 中大于等于 num 的第一个数的下标
    function findNearest(num: number, arr: number[]): number {
    ...
    // 找出某日及之后，第一个可用于抽水的日期，并将其使用（从 canDryDays 删去）
    function dryPoolAfterDay(pool: number, day: number): number {
        const dryDayIndex = findNearest(day, canDryDays);
        if (dryDayIndex === -1) return -1;

        const [dryDay] = canDryDays.splice(dryDayIndex, 1);
        return dryDay;
    }

    // 顺序处理每个日期
    for (let day = 0; day < rains.length; day++) {
        const pool = rains[day];

        // 遇到可抽水的日期，先不决定怎么使用，先存下
        if (pool === 0) {
            canDryDays.push(day);
            continue;
        }
        ...
        // 如果有值，说明这个 pool 在上次（lastDay）下过雨，必须在这期间找到可用的抽水日期抽水
        const lastDay = map[pool];

        // 寻找可抽水的日期，用第一个可用日期，因为遍历是从早到晚的，越靠后的日期越有价值（越可能被后续所需要）
        const dryDay = dryPoolAfterDay(pool, lastDay);
        if (dryDay === -1) {
            return [];
        }
