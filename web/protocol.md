...
比如使用whistle等抓包工具抓取https请求，本质上就是把抓包工具变成中间人角色，所以必定有一步前提是需要手动安装抓包工具提供的根证书。如果系统证书不被侵入，则https请求可认为绝对安全。
...
以上过程的关键是取证书信息和公钥这一步，并非从请求中获取（因为这步也可能被篡改），而是从浏览器或系统内置的CA获取，保证证书和公钥的绝对正确。
